...
{
    public class HexagonGridDrawer : IGridDrawer
    {
        public void DrawGrid(GameObject lineContainer, int gridSize, float cellSize, Color lineColor, float lineWidth)
        {
            ...
            for (int i = 0; i <= gridSize / 2 + 1; i++)
            {
                for (int j = 0; j <= gridSize / 2 + 1; j++)
                {
                    ...
                    DrawHexagon(lineContainer, center, cellSize, lineColor, lineWidth);
                }
            }
        }

        private static void DrawHexagon(GameObject lineContainer, Vector3 center, float cellSize, Color lineColor, float lineWidth)
        {
            for (int i = 0; i < 6; i++)
            {
